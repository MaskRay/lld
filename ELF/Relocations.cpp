...
static void scanRelocs(InputSectionBase &Sec, ArrayRef<RelTy> Rels) {
  ...
  if (Config->EMachine == EM_RISCV) {
    ...
    // addend for more efficient --toc-optimize processing.
    if (Sec.Name == ".toc") {
      llvm::stable_sort(Sec.Relocations,
                        [](const Relocation &LHS, const Relocation &RHS) {
                          return LHS.Offset < RHS.Offset;
      ...
      auto It = llvm::partition(Sec.Relocations, [](const Relocation &R) {
        return R.Expr == R_PPC64_RELAX_TOC;
      });
      std::stable_sort(Sec.Relocations.begin(), It,
                       [](const Relocation &LHS, const Relocation &RHS) {
