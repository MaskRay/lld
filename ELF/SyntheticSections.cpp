...
void MipsGotSection::build() {
  ...
  for (FileGot &Got : Gots) {
    ...
    for (std::pair<Symbol *, size_t> &P : Got.Tls) {
      ...
      if (S->IsPreemptible)
        mainPartition().RelaDyn->addReloc(Target->TlsGotRel, this, Offset, S);
    }
    for (std::pair<Symbol *, size_t> &P : Got.DynTlsSymbols) {
      ...
      if (S == nullptr) {
        if (!Config->Pic)
          continue;
        mainPartition().RelaDyn->addReloc(Target->TlsModuleIndexRel, this,
                                          Offset, S);
      } else {
        ...
        mainPartition().RelaDyn->addReloc(Target->TlsOffsetRel, this, Offset, S);
      }
    }
    ...
    for (const std::pair<Symbol *, size_t> &P : Got.Global) {
      uint64_t Offset = P.second * Config->Wordsize;
      mainPartition().RelaDyn->addReloc(Target->RelativeRel, this, Offset,
                                        P.first);
    }
    if (!Config->Pic)
      continue;
    ...
    for (const std::pair<GotEntry, size_t> &P : Got.Local16) {
      uint64_t Offset = P.second * Config->Wordsize;
      mainPartition().RelaDyn->addReloc({Target->RelativeRel, this, Offset,
                                         true, P.first.first, P.first.second});
    }
  }
}
...
void SymbolTableBaseSection::finalizeContents() {
  ...
  if (this == mainPartition().DynSymTab) {
    size_t I = 0;
...
void PartitionIndexSection::finalizeContents() {
  for (size_t I = 1; I != Partitions.size(); ++I)
    Partitions[I].NameStrTab =
        mainPartition().DynStrTab->addString(Partitions[I].Name);
}

void PartitionIndexSection::writeTo(uint8_t *Buf) {
  uint64_t VA = getVA();
  for (size_t I = 1; I != Partitions.size(); ++I) {
    write32(Buf, mainPartition().DynStrTab->getVA() + Partitions[I].NameStrTab - VA);
