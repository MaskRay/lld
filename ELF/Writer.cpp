...
template <class ELFT> void Writer<ELFT>::run() {
  ...
  Script->allocateHeaders(mainPartition().Phdrs);
...
template <class ELFT> void Writer<ELFT>::finalizeSections() {
  ...
  if (mainPartition().Dynamic->Parent)
    Symtab->addSymbol(
        Defined{/*File=*/nullptr, "_DYNAMIC", STB_WEAK, STV_HIDDEN, STT_NOTYPE,
                /*Value=*/0, /*Size=*/0, mainPartition().Dynamic});
  ...
  if (!Config->Relocatable && !Config->OFormatBinary) {
    ...
    Out::ProgramHeaders->Size = sizeof(Elf_Phdr) * mainPartition().Phdrs.size();
    ...
    for (PhdrEntry *P : mainPartition().Phdrs)
      if (P->p_type == PT_TLS)
        Out::TlsPhdr = P;
  }
...
template <class ELFT> void Writer<ELFT>::writeHeader() {
  writeEhdr<ELFT>(Out::BufferStart, mainPartition());
  writePhdrs<ELFT>(Out::BufferStart + sizeof(Elf_Ehdr), mainPartition());
    ...
    for (PhdrEntry *P : Part.Phdrs)
      if (P->p_type == PT_LOAD && (P->p_flags & PF_X))
        fillTrap(Out::BufferStart + alignDown(P->p_offset + P->p_filesz,
                 ...
                 Out::BufferStart + alignTo(P->p_offset + P->p_filesz,
                                            Config->CommonPageSize));
...
template <class ELFT> void Writer<ELFT>::writeBuildId() {
  if (!mainPartition().BuildId || !mainPartition().BuildId->getParent())
    return;
  ...
  size_t HashSize = mainPartition().BuildId->HashSize;
