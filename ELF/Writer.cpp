...
template <class ELFT> void Writer<ELFT>::setPhdrs() {
  for (PhdrEntry *P : Phdrs) {
    ...
    if (P->p_type == PT_TLS && P->p_memsz) {
      // On Android ARM/AArch64, reserve extra space (8 words) between the
      // thread pointer and an executable's TLS segment by overaligning the
      // segment. This reservation is needed for backwards compatibility with
      // Android's TCB, which allocates several slots after the thread pointer
      // (e.g. TLS_SLOT_STACK_GUARD==5).
      if (Config->AndroidTls && !Config->Shared &&
          (Config->EMachine == EM_ARM || Config->EMachine == EM_AARCH64))
        P->p_align = std::max<uint32_t>(P->p_align, Config->Wordsize * 8);
